...
// Helpers
const isNonEmpty = (v: string): boolean => typeof v === 'string' && v.trim().length > 0;
...
const EventSchema = new Schema<Event>(
  {
    ...
    description: {
      ...
      trim: true,
      validate: { validator: isNonEmpty, message: 'Description cannot be empty' },
    },
    overview: {
      type: String,
    ...
    agenda: {
      ...
      validate: {
        validator: (v: string[]) => Array.isArray(v) && v.length > 0 && v.every(isNonEmpty),
...
// Unique index for slug (redundant to slug.unique but explicit for safety)
